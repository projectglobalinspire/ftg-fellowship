...
test('application pages use precompiled Tailwind CSS instead of the browser compiler', async () => {
  for (const file of htmlFiles) {
    ...
    if (!/app\.js\?v=93/.test(html)) continue;
...
test('mentor identity follows the authenticated profile and incomplete mentors are gated', async () => {
  ...
  for (const file of ['mentor-dashboard.html', 'mentor-mentee.html', 'mentor-review.html']) {
    assert.match(await text(file), /app\.js\?v=93/);
  }
});
...
test('all dashboard shells follow the real viewport without a 900px scroll tail', async () => {
  ...
  for (const page of htmlFiles) {
    ...
    if (/main\s+data-design-id=/.test(html)) {
      assert.match(html, /responsive\.css\?v=89/, `${page} must load the no-scroll-tail shell`);
    }
  }
...
test('all dashboard headers use one bounded copy and action shell', async () => {
  const app = await text('app.js');
  const responsive = await text('responsive.css');
  assert.match(app, /function normalizeDashboardHeader\(header\)/);
  assert.match(app, /copy\.classList\.add\('ftg-header-copy'\)/);
  assert.match(app, /header\.classList\.add\('ftg-dashboard-header'\)/);
  assert.match(app, /\(normalizeDashboardHeader\(header\) \|\| header\)\.appendChild\(btn\)/);
  assert.doesNotMatch(app, /header\.style\.display = 'flex'/);
  assert.match(app, /actions\.insertBefore\(b,actions\.firstChild\)/);
  assert.match(responsive, /\.ftg-dashboard-header\s*\{[\s\S]*grid-template-columns:minmax\(0,1fr\) auto/);
  assert.match(responsive, /\.ftg-header-copy\s*\{[^}]*min-width:0/);
  ...
  assert.match(responsive, /\.ftg-header-actions i\s*\{[^}]*font-size:14px!important/);
});
...
test('workshop dates are centrally editable by Fasil and synchronized to every LMS role', async () => {
  ...
  assert.match(await text('workshop-library.html'), /app\.js\?v=93/);
...
test('public impact pages are bilingual, multi-program, privacy-safe and managed by Fasil', async () => {
  ...
  const programApi = await text('api/program.js');
  assert.match(await text('admin-program.html'), /app\.js\?v=93/);
  assert.match(await text('admin-program.html'), /responsive\.css\?v=89/);
...
test('track pairing, admin-completed mentors and mentee announcements are server backed', async () => {
  ...
  assert.match(css, /Mentee announcement: compact green program bulletin with a stable navigation rail/);
  assert.match(css, /grid-template-columns:minmax\(0,1fr\) 230px/);
  assert.match(css, /\.ftg-mentee-announcement-poster img\{width:100%;height:100%;object-fit:contain/);
  assert.match(css, /\.ftg-mentee-announcement\{grid-template-columns:1fr;min-height:0;height:auto\}/);
...
test('Fasil dialogs keep controls separated, closable and responsive', async () => {
  ...
  assert.match(css, /\.ftg-modal-toolbar\{[\s\S]*?position:absolute;[\s\S]*?top:14px;[\s\S]*?right:14px;/);
  assert.match(css, /\.ftg-modal-close\{\s*position:static!important/);
  assert.match(app, /box\.classList\.add\('ftg-profile-dialog'\)/);
  assert.match(css, /\.ftg-profile-dialog\{width:min\(560px/);
  assert.match(app, /var actions=normalizeDashboardHeader\(header\)\|\|header;actions\.insertBefore\(label,actions\.firstChild\)/);
  assert.doesNotMatch(app, /header\.lastElementChild\|\|header/);
  ...
  assert.match(app, /class="ftg-submission-workspace"/);
  assert.match(app, /data-submission-mentee/);
  assert.match(app, /localeCompare\(String\(b\.full_name\|\|''\),'id'/);
  assert.match(app, /submission_versions/);
  assert.match(app, /fa-google-drive/);
  assert.match(app, /box\.classList\.add\('ftg-submission-dialog'\)/);
  assert.match(css, /\.ftg-submission-dialog,\.ftg-modal-box:has\(\.ftg-assignment-monitor\)\{width:min\(1120px,calc\(100vw - 32px\)\)!important;max-width:[^}]+!important/);
  assert.match(css, /\.ftg-submission-workspace\{display:grid;flex:1;grid-template-columns:minmax\(260px,300px\) minmax\(480px,1fr\)/);
  assert.doesNotMatch(css, /\.ftg-monitor-create\{[^}]*margin-top:\s*-/);
});

test('operations overview returns complete mentee answers and version history to fasil', async () => {
  const operations = await text('api/operations.js');
  assert.match(operations, /text_content,link_url,files,checklist_state,status/);
  assert.match(operations, /submission_versions\?select=id,submission_id,version_number,text_content,link_url,files,created_at/);
  assert.match(operations, /assignment_targets\(mentee_id\)/);
  assert.match(operations, /submission_versions:submissionVersions/);
});
...
test('mentee announcements rotate every ten seconds with accessible controls', async () => {
  ...
  assert.doesNotMatch(app, /Berpindah otomatis setiap 10 detik/);
