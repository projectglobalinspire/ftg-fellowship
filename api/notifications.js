...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const profiles = targets.length ? await adminFetch(`/rest/v1/profiles?id=in.(${targets.map(encodeURIComponent).join(',')})&select=id,mentor_id`) : [];
    ...
    await adminFetch('/rest/v1/notifications', { method: 'POST', headers: { Prefer: 'return=minimal' }, body: JSON.stringify(clean) });
    return send(res, 201, { ok: true, sent: clean.length });
