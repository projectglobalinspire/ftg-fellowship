...
const safeUrl = value => /^https:\/\//i.test(String(value || '')) ? clean(value, 1200) : '';
...
function ensureTrustFields(program) {
  const copy=program&&typeof program==='object'?program:{};
  copy.finance=Object.assign({currency:'IDR',status:'draft',categories:[],monthly:[]},copy.finance||{});
  copy.sroi=Object.assign({sources:[],trend:[],respondents:0,baseline:'',attribution:0,deadweight:0,displacement:0,dropoff:0,scenario_low:0,scenario_high:0,limitations:''},copy.sroi||{});
  copy.organization=Object.assign({legal_name:'',registration_number:'',address:'',website:'',contact_email:'',leaders:[],partners:[],policies:[]},copy.organization||{});
  copy.targets=Array.isArray(copy.targets)?copy.targets:[];
  copy.timeline=Array.isArray(copy.timeline)?copy.timeline:[];
  copy.risks=Array.isArray(copy.risks)?copy.risks:[];
  copy.stories=Array.isArray(copy.stories)?copy.stories:[];
  copy.actions=Object.assign({proposal_url:'',meeting_url:'',contact_url:'https://wa.me/628111945700'},copy.actions||{});
  return copy;
}
...
function defaultPortal(programName) {
  return {
    ...
    programs:[{
      ...
      finance:{ currency:'IDR', contribution:100000000, spent:82000000, social_value:470000000, status:'draft', verified:false, note:'Angka awal dari rancangan donor; Fasil wajib memverifikasi sebelum publikasi.', note_en:'Initial dashboard figures; Fasil verification is required before publication.', categories:[], monthly:[] },
      ...
      sroi:{ ratio:4.7, methodology:'Social Value International / SROI Network Standard', period:'Jan–Jun 2026', verified:false, respondents:0, baseline:'', attribution:0, deadweight:0, displacement:0, dropoff:0, scenario_low:0, scenario_high:0, limitations:'', sources:[{label:'Peningkatan pendapatan',label_en:'Income uplift',value:210000000},{label:'Kesiapan kerja',label_en:'Employment readiness',value:135000000},{label:'Usaha baru',label_en:'New ventures',value:85000000},{label:'Nilai relawan mentor',label_en:'Mentor volunteer value',value:40000000}], trend:[3.1,3.8,4.3,4.7] },
      csr:{ pillars:[
      ...
      organization:{legal_name:'Faith to Grow',registration_number:'',address:'',website:'https://faithtogrow.org/',contact_email:'',leaders:[],partners:[],policies:[]},
      targets:[], timeline:[], risks:[], stories:[], actions:{proposal_url:'',meeting_url:'',contact_url:'https://wa.me/628111945700'}, beneficiaries:[], reports:[]
    }],
...
async function loadState() {
  ...
  portal.programs = Array.isArray(portal.programs) ? portal.programs.map(ensureTrustFields) : [];
...
function sanitizeProgram(input, existing={}) {
  ...
  const financeInput=input.finance||{},sroiInput=input.sroi||{},organizationInput=input.organization||{},actionsInput=input.actions||{};
  return {
    ...
    finance:{ currency:clean(financeInput.currency||'IDR',8).toUpperCase(), contribution:number(financeInput.contribution), spent:number(financeInput.spent), social_value:number(financeInput.social_value), status:['draft','verified','audited'].includes(financeInput.status)?financeInput.status:(financeInput.verified?'verified':(existing.finance&&existing.finance.status)||'draft'), verified:financeInput.status?financeInput.status!=='draft':Boolean(financeInput.verified), note:clean(financeInput.note,800), note_en:clean(financeInput.note_en,800), categories:Array.isArray(financeInput.categories)?financeInput.categories.slice(0,30).map(row=>({label:clean(row.label,100),budget:number(row.budget),actual:number(row.actual),evidence_url:safeUrl(row.evidence_url)})):[], monthly:Array.isArray(financeInput.monthly)?financeInput.monthly.slice(0,24).map(row=>({label:clean(row.label,30),budget:number(row.budget),actual:number(row.actual)})):[] },
    ...
    sroi:{ ratio:number(sroiInput.ratio,0,10000), methodology:clean(sroiInput.methodology,500), period:clean(sroiInput.period,80), verified:Boolean(sroiInput.verified), respondents:number(sroiInput.respondents,0,1000000), baseline:clean(sroiInput.baseline,1000), attribution:number(sroiInput.attribution,0,100), deadweight:number(sroiInput.deadweight,0,100), displacement:number(sroiInput.displacement,0,100), dropoff:number(sroiInput.dropoff,0,100), scenario_low:number(sroiInput.scenario_low,0,10000), scenario_high:number(sroiInput.scenario_high,0,10000), limitations:clean(sroiInput.limitations,1500), sources:Array.isArray(sroiInput.sources)?sroiInput.sources.slice(0,20).map(s=>({label:clean(s.label,100),label_en:clean(s.label_en,100),value:number(s.value)})):[], trend:Array.isArray(sroiInput.trend)?sroiInput.trend.slice(0,12).map(x=>number(x,0,10000)):[] },
    ...
    organization:{legal_name:clean(organizationInput.legal_name,180),registration_number:clean(organizationInput.registration_number,120),address:clean(organizationInput.address,500),website:safeUrl(organizationInput.website),contact_email:/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(String(organizationInput.contact_email||''))?clean(organizationInput.contact_email,254):'',leaders:Array.isArray(organizationInput.leaders)?organizationInput.leaders.slice(0,30).map(row=>({name:clean(row.name,120),role:clean(row.role,120)})):[],partners:Array.isArray(organizationInput.partners)?organizationInput.partners.slice(0,50).map(row=>clean(row,120)).filter(Boolean):[],policies:Array.isArray(organizationInput.policies)?organizationInput.policies.slice(0,30).map(row=>({label:clean(row.label,120),url:safeUrl(row.url)})):[]},
    targets:Array.isArray(input.targets)?input.targets.slice(0,50).map((row,index)=>({key:id(row.key)||`metric-${index+1}`,label:clean(row.label,120),target:number(row.target),actual:number(row.actual),unit:clean(row.unit,30),source:clean(row.source,160),formula:clean(row.formula,500),evidence_url:safeUrl(row.evidence_url),updated_at:clean(row.updated_at,40)})):[],
    timeline:Array.isArray(input.timeline)?input.timeline.slice(0,100).map((row,index)=>({id:id(row.id)||`${programId}-milestone-${index+1}`,date:clean(row.date,30),title:clean(row.title,160),status:['upcoming','progress','completed','delayed'].includes(row.status)?row.status:'upcoming',description:clean(row.description,800),evidence_url:safeUrl(row.evidence_url)})):[],
    risks:Array.isArray(input.risks)?input.risks.slice(0,100).map((row,index)=>({id:id(row.id)||`${programId}-risk-${index+1}`,title:clean(row.title,160),level:['low','medium','high','critical'].includes(row.level)?row.level:'medium',status:['open','monitored','mitigated'].includes(row.status)?row.status:'open',mitigation:clean(row.mitigation,800),owner:clean(row.owner,120),updated_at:clean(row.updated_at,40)})):[],
    stories:Array.isArray(input.stories)?input.stories.slice(0,100).map((row,index)=>({id:id(row.id)||`${programId}-story-${index+1}`,alias:clean(row.alias,80),title:clean(row.title,160),quote:clean(row.quote,1000),baseline:clean(row.baseline,800),outcome:clean(row.outcome,800),evidence_url:safeUrl(row.evidence_url),consent:Boolean(row.consent)})).filter(row=>row.consent):[],
    actions:{proposal_url:safeUrl(actionsInput.proposal_url),meeting_url:safeUrl(actionsInput.meeting_url),contact_url:safeUrl(actionsInput.contact_url)},
    beneficiaries:Array.isArray(input.beneficiaries)?input.beneficiaries.slice(0,500).map((b,index)=>({id:id(b.id)||`${programId}-beneficiary-${index+1}`,name:clean(b.name,120),initials:clean(b.initials,4),path:clean(b.path,80),status:clean(b.status,30)||'active',progress:number(b.progress,0,100),outcome:clean(b.outcome,500),bio:clean(b.bio,800),avatar_url:safeUrl(b.avatar_url),profile_id:/^[0-9a-f-]{36}$/i.test(String(b.profile_id||''))?b.profile_id:null,public_consent:Boolean(b.public_consent)})):[],
    reports:Array.isArray(input.reports)?input.reports.slice(0,100).map((r,index)=>({id:id(r.id)||`${programId}-report-${index+1}`,title:clean(r.title,160),title_en:clean(r.title_en,160),type:clean(r.type,40),period:clean(r.period,80),url:safeUrl(r.url),verified:Boolean(r.verified)})):[]
  };
}
...
async function hydratePrograms(programs) {
  ...
  hydrated.forEach(program=>{if(program.source!=='ftg'||!program.analytics)return;const existingTargets=new Map((program.targets||[]).map(row=>[row.key,row])),targetFor=(key,fallback)=>number(existingTargets.get(key)&&existingTargets.get(key).target||fallback),stamp=program.analytics.synced_at;const liveTargets=[
    {key:'beneficiaries',label:'Penerima manfaat',target:targetFor('beneficiaries',program.beneficiary_target),actual:program.impact.beneficiaries,unit:'orang',source:'Profil aktif LMS',formula:'Jumlah profil mentee berstatus aktif',updated_at:stamp},
    {key:'active_rate',label:'Peserta aktif 14 hari',target:targetFor('active_rate',80),actual:program.impact.active_rate,unit:'%',source:'Aktivitas LMS',formula:'Mentee aktif dalam 14 hari ÷ seluruh mentee aktif',updated_at:stamp},
    {key:'submission_rate',label:'Tingkat pengumpulan',target:targetFor('submission_rate',90),actual:program.impact.completion_rate,unit:'%',source:'Tugas dan pengumpulan LMS',formula:'Tugas terkumpul ÷ pasangan tugas-peserta yang ditargetkan',updated_at:stamp},
    {key:'attendance_rate',label:'Tingkat kehadiran',target:targetFor('attendance_rate',85),actual:program.analytics.engagement.attendance_rate,unit:'%',source:'Presensi kegiatan',formula:'Hadir atau terlambat ÷ seluruh catatan presensi',updated_at:stamp},
    {key:'average_score',label:'Rata-rata nilai',target:targetFor('average_score',75),actual:program.analytics.quality.average_score,unit:'/100',source:'Review mentor',formula:'Jumlah skor review ÷ jumlah tugas yang telah dinilai',updated_at:stamp}
  ];program.targets=liveTargets.concat((program.targets||[]).filter(row=>!liveTargets.some(live=>live.key===row.key)));});
