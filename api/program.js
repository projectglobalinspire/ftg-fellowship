...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if(body.action==='pairings_auto'){
      ...
      const remaining=mentees.filter(mentee=>!mentors.some(mentor=>mentor.path===mentee.path));
      const missing_by_track=remaining.reduce((out,mentee)=>{out[mentee.path||'Belum diatur']=(out[mentee.path||'Belum diatur']||0)+1;return out;},{});
      await adminFetch('/rest/v1/audit_logs',{method:'POST',headers:{Prefer:'return=minimal'},body:JSON.stringify({actor_id:auth.user.id,action:'pairing.auto_balance',entity_type:'profile',detail:{paired,unpaired:remaining.length,missing_by_track}})}).catch(()=>null);return send(res,200,{ok:true,paired,unpaired:remaining.length,missing_by_track});
    }
